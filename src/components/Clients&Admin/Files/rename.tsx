...
function  Rename(props: Props) {
    ...
    async function ChangeName(){
        ...
        try{
            updateDoc(doc(db, 'files', props.file.id_company, "Arquivos", props.file.id_file), {
                name: nameFile
            })
        ...
        console.log(e)
        toast.error("Não foi possivél alterar o nome deste arquivo.")
        }
    }
        ...
        <div className='bg-primary w-[400px] max-lsm:w-[320px] rounded-[4px] flex flex-col'>
          ...
          <div className='px-[10px]'>
            ...
            <div className='mt-[15px] '>
                ...
                <input maxLength={25} onChange={(text) => setNameFile(text.target.value)} className='w-full text-[20px] max-sm:text-[18px] bg-transparent border-[2px] border-black p-[3px] rounded-[4px] ' placeholder='Digite o novo nome'/>
            </div>
          </div>
          <div className='flex w-full justify-end gap-4 bg-hilight self-end pr-[10px] py-[10px] rounded-b-[4px] mt-[25px]'>
            ...
            <button  onClick={() => toast.promise(ChangeName(), messageToast)} className={`${nameFile.length > 0 ? "bg-[rgba(126,181,163,0.40)] border-[rgba(126,181,163,1)]": "bg-strong/30 border-strong text-white" } border-2 hover:scale-[1.10]  duration-300 py-[3px] px-[10px] rounded-[8px] text-[18px] text-white `}>Alterar</button>
          </div>
        </div>
      </div>
    )
