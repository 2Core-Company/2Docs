...
export function DocTableFooter({ files, textSearch, dataPages, setDataPages, unselectFunction }: DocTableFooterProps) {
    return(
        ...
        <div className='flex justify-between w-full h-[61px] max-sm:h-[30px] items-center px-[26px] mt-auto border-t border-t-secondary'>
            ...
            <button onClick={() => changePage({action: 'next', dataPages, setDataPages, unselectFunction})} className={`px-[14px] py-[5px] ${files.length / 10 <= dataPages.page ? "bg-[#D9D9D9] border border-[#9E9E9E] text-[#AAAAAA]" : "cursor-pointer bg-[#10B981] text-white"} rounded-[8px] text-[18px] max-md:text-[16px] max-lsm:text-[14px]`}>Proximo</button>
        </div> :
        <></>
    )
}