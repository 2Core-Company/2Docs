...
import { Files } from '../../../types/files'
 ...
 function Message({file, files, childToParentDownload}:Props) {
    ...
    const [message, setMessage] = useState<string | undefined>(file.message)
                ...
                <div className='bg-primary dark:bg-dprimary w-[400px] max-lsm:w-[320px] rounded-[4px] flex flex-col'>
                    <div  className='bg-[rgba(126,181,163,1)] w-full h-[15px] rounded-t-[4px]'/>
                        <div className='px-[10px] '>
                            <p className='text-[24px] max-lsm:text-[20px] mt-[10px] text-left'>{file?.message && file?.message?.length  > 0 ? "Edite a observação deste arquivo." : "Adicione uma observação ao arquivo."}</p>
                    ...
                    <div className='flex w-full justify-end gap-4 bg-hilight dark:bg-dhilight self-end pr-[10px] py-[10px] rounded-b-[4px] mt-[10px]'>
                        <button  onClick={() => setModalMessage({...modalMessage, permission:"", status:false})} className='bg-strong/40 dark:bg-dstrong/40 border-[2px] border-strong hover:scale-[1.10] duration-300 p-[3px]  rounded-[8px] text-[18px] text-white '>Cancelar</button>
