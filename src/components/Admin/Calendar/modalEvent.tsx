...
function ModalEvent({ emailUser, action, event, defaultValue, modalEvent, setModalEvent, childModalEvent }: Props) {
    ...
    async function CreateEventInFireStore() {
        const result = await CreateEvent({ event: {...dataEvent, tasks}, id_company: dataAdmin.id_company, email })
    ...
    return (
        <Dialog.Root open={modalEvent} onOpenChange={setModalEvent}>
            <Dialog.Portal>
                ...
                <Dialog.Content id={style.boxModalEvent} className=" data-[state=open]:animate-contentShow fixed top-[50%] rounded-[15px] left-[50%] w-[530px] max-sm:w-[400px] max-lsm:w-[360px] overflow-hidden translate-x-[-50%] translate-y-[-50%] focus:outline-none z-20 max-h-[95%]">
                        ...
                        <div>
                            <div className='text-left flex flex-col mt-[20px] max-sm:mt-[10px] px-[40px] max-sm:px-[20px] '>
                                ...
                                {tasks.map((task, index) => {
                                    return (
                                        <div className='flex items-center border-b border-neutral-400 py-1 my-2'>
