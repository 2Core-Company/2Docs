...
interface Props {
    ...
    setModalCreateEvent:Function
}

function ModalCreateEvent({modalCreateEvent, setModalCreateEvent}:Props) {
    ...
    return (
        <Dialog.Root open={modalCreateEvent}>
            <Dialog.Portal>
                <Dialog.Overlay onClick={() => setModalCreateEvent(false)} className="bg-blackA9 data-[state=open]:animate-overlayShow fixed inset-0 z-20" />
                <Dialog.Content className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] w-[530px] max-sm:w-[400px] max-lsm:w-[350px] rounded-[15px] translate-x-[-50%] translate-y-[-50%] bg-primary shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none z-20 max-h-[95%] overflow-y-auto">
                    ...
                    <div className='text-left flex flex-col mt-[20px] max-sm:mt-[10px] px-[40px] max-sm:px-[20px]'>
                        <p className='text-[26px] max-sm:text-[24px] max-lsm:text-[22px] after:w-[40px] after:h-[3px] after:block after:bg-blue after:rounded-full after:ml-[3px] after:mt-[-5px]'>Criar Evento</p>
                        ...
                        <textarea maxLength={250} disabled={loading} rows={4} placeholder='Digite a mensagem do evento' onChange={(text) => setDataEvent({...dataEvent, description:text.target.value})} className='text-[18px] max-sm:text-[16px] resize-none mt-[4px] w-full px-[15px] py-[8px] rounded-[8px] border-[1px] border-black text-[#9E9E9E] bg-transparent focus:outline-none focus:ring-[2px] focus:ring-[#686868]'/>
                        ...
                        <div className='flex items-center mt-[15px]'>
                            <input disabled={loading} value={dataEvent.definedDate.toString()}  onChange={(e) => setDataEvent({...dataEvent, definedDate:!dataEvent.definedDate, dateEnd:null})} type="checkbox" className={`${dataEvent.definedDate ? '' : 'appearance-none'}  accent-gray-600 w-[20px] h-[20px] border-[1px] border-[#686868] rounded-[3px] cursor-pointer max-sm:w-[18px] max-sm:h-[18px]`} />
                        ...
                        <div className='flex items-center mt-[10px]'>
                            <input disabled={loading} value={dataEvent.limitedDelivery.toString()} onChange={(e) => setDataEvent({...dataEvent, limitedDelivery:!dataEvent.limitedDelivery})} type="checkbox" className={`${dataEvent.limitedDelivery ? '' : 'appearance-none'}  accent-gray-600 w-[20px] h-[20px] border-[1px] border-[#686868] rounded-[3px] cursor-pointer max-sm:w-[18px] max-sm:h-[18px]`} />
                    ...
                    <div className='bg-[#D9D9D9] mt-[15px] px-[30px] max-sm:px-[15px] py-[14px] flex justify-between font-[500] rounded-b-[15px]'>
                        <Dialog.Close asChild>
                            <button disabled={loading} onClick={() => setModalCreateEvent(false)} className='text-[#EBEBEB] bg-[#8F8F8F] rounded-[8px] px-[14px] py-[9px] hover:bg-[#777777] duration-100'>
                                Cancelar
                            </button>
                        </Dialog.Close>
