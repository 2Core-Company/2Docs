...
function TableClients({users, setUsersFilter, usersFilter, pages, windowsAction, SelectUsers,setUserEdit,setWindowsAction}: Props) {
      ...
      {usersFilter.length > 0 ? (
        <>
          ...
          <div className="w-full mt-[10px] grid grid-cols-[20px__repeat(2,1fr)_200px_65px_60px] max-lg:grid-cols-[20px__repeat(2,1fr)_65px_60px] max-md:grid-cols-[20px__1fr_65px_60px] px-[5px] gap-x-[15px] text-[18px] font-[500] border-y-[1px] border-y-neutral-400  bg-neutral-300  items-center py-[5px]">
            ...
            <button onClick={() => ( setFilter({...filter, status: !filter.status, name: false, date: false}), FilterStatus({dataFilter:usersFilter, filter:filter, setReturn:setUsersFilter}))} className="flex items-center cursor-pointer">
              ...
              if (showItens.min < index && index < showItens.max) {
                return (
                  <div key={index} className="w-full gap-y-[5px] grid grid-cols-[20px__repeat(2,1fr)_200px_65px_60px] max-lg:grid-cols-[20px__repeat(2,1fr)_65px_60px] max-md:grid-cols-[20px__1fr_65px_60px] border-b-[1px] border-b-neutral-400 px-[5px] gap-x-[15px] text-[18px] font-[500] items-center py-[5px]">
                    <input aria-label="Selecionar Usuário" type="checkbox" checked={checked} onChange={(e) => (checked = e.target.value === "on" ? true : false)} onClick={() => SelectUsers(index)} className="w-full h-[20px]"/>
