...
function TableClients({
  ...
  const [showItens, setShowItens] = useState<{ min: number; max: number }>({
    min: -1,
    max: 10,
  });
  const [filter, setFilter] = useState<{
    ...
    date: boolean;
    status: boolean;
  }>({ name: false, date: false, status: false });
  ...
  function formatDate(date: string) {
    ...
    if (window.screen.width > 1250) {
      ...
      return (
        date.substr(8, 2) + " de " + months[month] + " de " + date.substr(11, 4)
      );
    } else {
    ...
    <>
      {usersFilter.length > 0 ? (
          ...
          <thead>
            <tr className="bg-[#DDDDDD] dark:bg-[#fff]/5 border-b-[2px] border-t-[2px] border-terciary dark:border-dterciary text-[20px] max-lg:text-[18px] max-md:text-[17px]">
              <th className="py-[10px]">
                <input
                  aria-label="checkbox demonstrativo"
                  type="checkbox"
                  disabled={true}
                  className="w-[20px] h-[20px] all-unset"
                />
              </th>
              ...
              <th className="font-[400] text-left pl-[20px] max-lg:pl-[10px]">
                ...
                >
                  ...
                  <Image
                    ...
                    className={`ml-[5px] ${filter.name ? "rotate-180" : ""}`}
                    src={ArrowFilter}
                  />
                </button>
              </th>

              <th className="font-[400] text-left pl-[20px] max-md:hidden dark:text-white">
                Email
              </th>
              ...
              <th className="font-[400] max-lg:hidden">
                ...
                >
                  ...
                  <Image
                    alt="Imagem de uma flecha"
                    className={`ml-[5px] ${filter.date ? "rotate-180" : ""}`}
                    src={ArrowFilter}
                  />
                </button>
              </th>
              ...
              <th className="font-[400]">
                ...
                >
                  ...
                  <Image
                    alt="Imagem de uma flecha"
                    className={`ml-[5px]  ${filter.status ? "rotate-180" : ""}`}
                    src={ArrowFilter}
                  />
                </button>
              </th>
            ...
            {usersFilter.map((user: DataUser, index) => {
              ...
              if (showItens.min < index && index < showItens.max) {
                return (
                  <tr
                    key={user.id}
                    className={`border-b-[1px] border-terciary dark:border-dterciary text-[18px] max-lg:text-[16px] ${
                      user.fixed ? "bg-neutral-300 bg-neutral-300/10" : ""
                    }`}
                  >
                    <th className="h-[50px] max-sm:h-[40px]">
                      <input
                        ...
                        onClick={() => SelectUsers(index)}
                        className="w-[20px] h-[20px]  max-sm:w-[15px] max-sm:h-[15px] ml-[5px]"
                      />
                    </th>
                    ...
                    <th className="font-[400] flex ml-[20px] max-lg:ml-[10px] items-center h-[50px] max-sm:h-[40px]">
                      <Image
                        ...
                        height={40}
                        alt="Perfil"
                        className="text-[10px] mt-[3px] rounded-full mr-[10px] max-w-[40px] min-w-[40px] min-h-[40px] max-h-[40px]  max-md:min-w-[30px] max-md:max-w-[30px]  max-md:min-h-[30px] max-md:max-h-[30px]"
                      />
                      <p className="overflow-hidden whitespace-nowrap text-ellipsis max-w-[180px] max-lg:max-w-[130px] max-lsm:max-w-[80px] dark:text-white">
                        {user.name}
                      </p>
                    </th>

                    <th className="font-[400] text-left pl-[20px] max-md:hidden">
                      <p className="overflow-hidden whitespace-nowrap text-ellipsis max-w-[250px] dark:text-white">
                        {user.email}
                      </p>
                    </th>

                    <th className="font-[400] max-lg:hidden text-left dark:text-white">
        ...
        <div className="w-full h-full flex justify-center items-center flex-col">
          <Image
            src={users.length <= 0 ? iconNullClient : iconSearchUser}
            width={80}
            height={80}
            onClick={() =>
              setWindowsAction({ ...windowsAction, createUser: true })
            }
            alt="Foto de uma mulher, clique para cadastrar um cliente"
            className="cursor-pointer w-[170px] h-[170px]"
          />
            ...
            <button
              onClick={() => {
                showItens.max / 10 != 1
                  ? setShowItens({
                      ...showItens,
                      min: showItens.min - 10,
                      max: showItens.max - 10,
                    })
                  : "";
              }}
              className={` border-[2px] ${
                showItens.max / 10 == 1
                  ? "bg-hilight dark:bg-dhilight border-terciary dark:border-dterciary text-terciary dark:text-dterciary"
                  : "bg-black dark:bg-white border-black dark:border-white text-white dark:text-black"
              } p-[4px] max-sm:p-[2px] rounded-[8px] text-[18px] max-md:text-[16px] max-lsm:text-[14px]`}
            >
              Anterior
            </button>
            <p className="dark:text-white">{`Página ${
              showItens.max / 10
            } de ${pages}`}</p>
            <button
              onClick={() => {
                showItens.max / 10 != pages
                  ? setShowItens({
                      ...showItens,
                      min: showItens.min + 10,
                      max: showItens.max + 10,
