    ...
    <div className={`w-[600px] z-10 max-sm:z-50 max-sm:w-screen absolute bg-[#DDDDDD] dark:bg-[#121212] min-h-screen pb-[100px] right-0 duration-300 flex flex-col items-center top-0 drop-shadow-[0_0px_10px_rgba(0,0,0,0.50)]`}>
        ...
        <form  onSubmit={OnToast} className='w-full px-[10%] flex flex-col gap-y-[20px] max-sm:gap-y-[5px] text-[20px] max-sm:text-[18px]'>
          ...
          <label  className='flex flex-col max-sm dark:text-white'>
            Nome
            <input autoComplete="off" type="text" maxLength={30} value={dataUser.name} required  onChange={(Text) => setDataUser({...dataUser, name:Text.target.value})}  className='outline-none w-full p-[5px] bg-transparent border-2 border-black dark:border-white rounded-[8px]' placeholder='Digite o nome do cliente'/>
          </label>

          <label className='flex flex-col dark:text-white'>
            Email
            <input autoComplete="off" required  value={dataUser.email} maxLength={40} onChange={(Text) => setDataUser({...dataUser, email:Text.target.value})} type="email"   className='outline-none w-full text-[18px] p-[5px] bg-transparent border-2 border-black dark:border-white rounded-[8px]' placeholder='Digite o email'/>
          </label>
