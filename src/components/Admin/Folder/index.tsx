...
function ComponentFolder() {
  ...
  async function GetUser() {
    ...
    if (id_enterprise) {
      const index = docSnap
        .data()
        .enterprises.findIndex((enterprise) => enterprise.id === id_enterprise);
    ...
    if (
      ...
      for (var i = 0; i < user.folders.length; i++) {
        if (
          user.folders[i].name
            .toLowerCase()
            .includes(searchFolders.toLowerCase().trim())
        ) {
  ...
  async function GetFiles() {
    const files = [];
    const q = query(
      collection(db, "files", context.dataUser.id_company, "Arquivos"),
      where("id_user", "==", id)
    );
  ...
  async function FilterDate(getFiles: Files[]) {
    const filesHere = [...getFiles].filter(
      (file) => file.trash === false && file.from === "user"
    );
    const recents = [];
    filesHere.sort((a, b) => {
  ...
  function ConfirmationDeleteFolder(name: string) {
    ...
    setModal({
      ...modal,
      status: true,
      message: "Tem certeza que deseja excluir está Pasta?",
      subMessage1: "Todos os arquivos irão para a lixeira.",
    });
  }
    ...
    for (var i in filesToTrash) {
      await updateDoc(
        doc(
          ...
          context.dataUser.id_company,
          "Arquivos",
          filesToTrash[i].id_file
        ),
        {
          trash: true,
        }
      );
      const index = filesHere.findIndex(
        (file) => file.id_file === filesToTrash[i].id_file
      );
      ...
      <div className="w-[85%] h-full ml-[100px] max-lg:ml-[0px] max-lg:w-[90%] mt-[50px]">
        {recentsFile.length > 0 ? (
          <>
            ...
            <div className="flex items-top">
              ...
              <p className="text-[18px] flex mx-[5px] text-secondary dark:text-dsecondary">
                {"Pastas"}
              </p>
            </div>
            ...
            <div className="flex flex-wrap mt-[30px]">
              {recentsFile.map((file) => {
                return (
                  <div
                    key={file.id_file}
                    className="group  w-[250px] max-md:w-[180px] max-sm:w-[150px] max-lsm:w-[120px] p-[10px] rounded-[8px] hover:scale-105 hover:shadow-[#dadada] hover:shadow-[0_5px_10px_5px_rgba(0,0,0,0.9)] dark:hover:shadow-[#414141] relative"
                  >
                    <button
                      onClick={() =>
                        DownloadsFile({
                    ...
                    <Image
                      ...
                      width={90}
                      height={90}
                      className="max-lg:h-[70px] max-lg:w-[70px] max-sm:h-[60px] max-sm:w-[60px] max-lsm:h-[50px] max-lsm:w-[50px]"
        ...
        <div className="w-[500px] max-md:w-[90%] flex justify-between">
          <label className="flex w-[80%] justify-center items-center">
            ...
            />
            <input
              onChange={(text) => setSearchFolders(text.target.value)}
              type="text"
              className="w-[90%] text-black dark:text-white bg-transparent text-[20px] outline-none max-sm:text-[14px] max-lsm:text-[12px] border-b-black dark:border-b-white border-b-[2px]"
              placeholder="Buscar"
            ></input>
          </label>
          <button
            className={`bg-black dark:bg-white cursor-pointer text-white dark:text-black p-[5px] flex justify-center items-center rounded-[8px] text-[17px] max-sm:text-[14px] max-lsm:text-[12px]`}
            ...
            foldersFilter.map((folder, index) => {
              if (
                  ...
                  folder.name === "Favoritos"
                    ? files.filter(
                        (file) =>
                          file.favorite === true &&
                          file.trash === false &&
                          file.id_enterprise === enterprise.id
                      )
                    : files.filter(
                        (file) =>
                ...
                return (
                  <div
                    key={folder.name}
                    className="group mt-[30px] w-[250px] max-md:w-[180px] max-sm:w-[150px] max-lsm:w-[120px] p-[10px] rounded-[8px] hover:scale-105 hover:shadow-[#dadada] dark:hover:shadow-[#414141] hover:shadow-[0_5px_10px_5px_rgba(0,0,0,0.9)]"
                  >
                    {folder.name === "Cliente" ||
                    folder.name === "Favoritos" ? (
                      <></>
        ...
        <Link
          href={{
            pathname: "/Admin/Arquivos",
            query: { trash: true, id: id, id_enterprise: enterprise?.id },
          }}
          className="w-[250px] max-md:w-[180px] max-sm:w-[150px] max-lsm:w-[120px] p-[10px] rounded-[8px]"
        >
          <div className="relative w-[90px] h-[90px] max-lg:h-[70px] max-lg:w-[70px] max-sm:h-[60px] max-sm:w-[60px] max-lsm:h-[50px] max-lsm:w-[50px]">
            <p className="font-500 text-[18px] w-[25px] h-[25px] bg-secondary dark:bg-dsecondary rounded-full absolute text-center text-white right-[-10px]">
              {FilterTrash().length}
