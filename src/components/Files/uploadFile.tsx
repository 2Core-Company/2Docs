  ...
  function UploadFiles(props:Props) {
    ...
    async function UploadFile(files){
      ...
      for await (const file of files.files) {
        ...
        const docsRef = ref(storage, `${props.id_company}/files/${props.id_user + "/" + referencesFile}`);
    ...
    return(
      <label className={`${props.folderName === "Cliente" ? "hidden" : <></>} bg-black cursor-pointer text-white p-[5px] flex justify-center items-center rounded-[8px] text-[17px] max-sm:text-[14px] ${props.menu ? "max-lg:hidden" : ""}`}>
        <p>Upload</p>
