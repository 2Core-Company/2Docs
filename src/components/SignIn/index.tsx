...
export function AlterPassword(email: string) {  

  if (email === "") {          
    toast.error("Preencha o campo de email.")
    throw new Error('Email value should be filled to proceed.')
  } else{
...
export async function AlterPasswordCnpj(cnpj: string) {
    if (cnpj === "") {
      toast.error("Preencha o campo de cnpj.")
      return 'CNPJ value should be filled to proceed.'
    } else{
            ...
            <fieldset className="flex flex-col mt-[20px]">
              ...
              <div className='flex pl-[5px] border-[1px] border-black rounded-[8px] items-center'>
                <input required minLength={8}
                       ...
                       onChange={(Text) => setDataUser({ ...dataUser, password: Text.target.value })} className="w-full text-[18px] bg-[#0000] outline-none py-[10px]" placeholder='Digite sua senha' />
